...
void NodeAttachment::NewIndex(const char* field)
{
	if (field)
	{
		...
		case 't':
			if (!strcmp(field, "tether"))
			{
				tether = static_cast<TetherType>(GetClampedInteger(-1, TetherType::TETHER_ROTATING, TetherType::TETHER_NONE));
				return;
			}
			break;
		}
...
void ParticleGroup::NewIndex(const char* field)
{
	...
	if (field)
	{
		...
		case 'b':
			if (!strcmp(field, "blendingMode"))
			{
				blendingMode = GetClampedInteger(-1, BlendMode::BLEND_TEXTURE, BlendMode::BLEND_CUSTOM_13);
				return;
			}
			break;
		case 'd':
			if (!strcmp(field, "drawMode"))
			{
				drawMode = static_cast<DrawMode>(GetClampedInteger(-1, DrawMode::DRAW_SPRITE, DrawMode::DRAW_NONE));
				return;
			}
			break;
		case 'l':
